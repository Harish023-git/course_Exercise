"""Problem - 3:
We can make  program run faster using a technique introduced in lecture - bisection search!
The following variables contain values as described below:

balance - the outstanding balance on the credit card
annualInterestRate - annual interest rate as a decimal

To recap the problem: we are searching for the smallest monthly payment such that we can pay off the entire balance within a 
year. What is a reasonable lower bound for this payment value? $0 is the obvious answer, but you can do better than that. 
...
compounded on the balance we didn't pay off each month. So a good upper bound for the monthly payment would be one-twelfth 
...
init_balance = balance = int(input("Enter Balance: "))
annualInterestRate = float(input("Enter Annual Interest Rate: "))
Monthly_interest_rate = (annualInterestRate)/12.0
Monthly_payment_lower_bound = init_balance/12
Monthly_payment_upper_bound = (init_balance * (1 + Monthly_interest_rate)**12)/12.0
epsilon = 0.01

while abs(balance) > epsilon:
    monthlyPaymentRate = (Monthly_payment_upper_bound + Monthly_payment_lower_bound)/2
    balance = init_balance
    for i in range(12):
        balance = balance - monthlyPaymentRate + ((balance - monthlyPaymentRate) * Monthly_interest_rate)
    if balance > epsilon:
        Monthly_payment_lower_bound = monthlyPaymentRate
    elif balance < -epsilon:
        Monthly_payment_upper_bound = monthlyPaymentRate
    else:
        break
